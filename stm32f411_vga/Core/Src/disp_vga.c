...
uint8_t vramr[80*480] __attribute__((aligned(32)));
uint8_t vramg[80*480] __attribute__((aligned(32)));
uint8_t vramb[80*480] __attribute__((aligned(32)));
...
void clr_vram(){
	memset(vramr, 0x00, 80*480);
	memset(vramg, 0x00, 80*480);
	memset(vramb, 0x00, 80*480);
}
...
void DrawChar(int xp, int yp, char code, uint8_t col){
	...
	if((col & 1)!=0)
		for( i=0; i<8; i++){
			...
			vramr[(yp+i*2+1)*80+xp] = fontptn[code*8+i];
		}
	if((col & 2)!=0)
		for( i=0; i<8; i++){
			vramg[(yp+i*2  )*80+xp] = fontptn[code*8+i];
			vramg[(yp+i*2+1)*80+xp] = fontptn[code*8+i];
		}
	if((col & 4)!=0)
		for( i=0; i<8; i++){
			vramb[(yp+i*2  )*80+xp] = fontptn[code*8+i];
			vramb[(yp+i*2+1)*80+xp] = fontptn[code*8+i];
		}
	if(col==0)
		for( i=0; i<8; i++){
			vramr[(yp+i*2  )*80+xp] = 0; vramr[(yp+i*2+1)*80+xp] = 0;
